    ...
    async def _stats_loop(self) -> None:
        ...
        while not self.bus.stop_event.is_set():
            ...
            if n or s["text"] or s["audio"] or s["listen"] or s["done"]:
                log.info(
                    "omni 5s: uplink=%d chunks (mic rms~%.4f, dropped=%d) | downlink text=%d audio=%d listen=%d done=%d | playq=%d",
                    n, (s["up_rms"] / n if n else 0.0), s["drop"], s["text"], s["audio"], s["listen"], s["done"], playq,
                )
                if playq > 25:
