...
- YRobot 使用 DoA 做朝向和连续的轻量姿态叠加；DoA 不能单独触发打断，只能与 XVF
  post-AEC 电平相对播放残余底噪的持续上升共同确认 double-talk。按 SDK 的实时控制原则使用
...
- MiniCPM-o 优先自主输出 `listen/speak`。播放开始后的 400 ms 用于学习 far-end 残余；此后
  只有 DoA speech（带 150 ms 保持）与 post-AEC 电平持续 120 ms 超过残余底噪至少 6 dB，
  才清空播放并发送 `force_listen`。该控制片段使用静音，真实麦克风片段保留到 `listen`
  确认后再正常发送，避免打断短句被控制决策吃掉。等待确认有 3 秒上限，超时只恢复正常
  输入；被打断轮次是"整轮废弃"——服务端一个 turn 会超前 10+ 秒突发送流，其音频一律
  丢弃到该轮的 `listen`/`response.done` 边界为止，绝不中途复活。新会话建立时清除遗留
  打断状态。安静的普通 `listen` 保留短缓冲句尾，已确认的用户插话则清空 GStreamer；
  `clear_player` 会对共享的录音+播放管线做 PAUSED→flush→PLAYING 循环，只允许播放线程
  （唯一推流者）执行，否则管线可能永久卡死。
- 播放预滚只作用于轮次起点：真实 TTS 供给缺口把预滚抬高到缺口加余量，每个完成的轮次
  衰减 20 ms，网络健康时首音等待收敛到 120 ms；回复中途断供立即续播且不重置重采样器，
  等待只会拉长可闻空洞。发送慢于 250 ms 时下一片自动只发音频不发视频。
