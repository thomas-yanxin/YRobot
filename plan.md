...
- MiniCPM-o 优先自主输出 `listen/speak`。播放开始后的 400 ms 用于学习 far-end 残余；此后
  ...
  才清空播放并发送 `force_listen`。打断确认的瞬间把采集缓冲里未满一秒的**真实用户语音**
  ...
  0.4 s arm + 120 ms 确认的重检延迟）；用户安静后的第一个 `listen` 边界恢复播放，
  12 秒上限防止环境噪声导致永久静音。安静的普通 `listen` 保留短缓冲句尾，已确认的用户插话则清空 GStreamer；
