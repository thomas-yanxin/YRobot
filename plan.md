...
- MiniCPM-o 优先自主输出 `listen/speak`。播放开始后的 400 ms 用于学习 far-end 残余；此后
  ...
  确认后再正常发送，避免打断短句被控制决策吃掉。等待确认有 3 秒上限，超时自动恢复正常
  输入与播放；新会话建立时也会清除遗留的打断状态，两者都防止确认丢失导致机器人永久静音。
  安静的普通 `listen` 保留短缓冲句尾，已确认的用户插话则清空 GStreamer。
- 播放预滚自适应：真实 TTS 供给缺口把预滚抬高到缺口加余量，每个完成的轮次衰减 20 ms，
  网络健康时首音等待逐步收敛到 120 ms。发送慢于 250 ms 时下一片自动只发音频不发视频。
