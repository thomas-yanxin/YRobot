...
- YRobot 同时需要 DoA、打断检测和连续的轻量姿态叠加，因此按 SDK 的实时控制原则使用唯一的
  `set_target()` 控制点；当前为 50 Hz，相位对齐并按实际 `dt` 限速。
...
- 用户说话同时连续触发 DoA speech VAD 与 AEC 后麦克风能量门限时，立即清空应用与 SDK
  播放队列；用户说话期间持续发送 `force_listen`，直到服务端确认监听后再恢复播放。
