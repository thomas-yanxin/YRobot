...
- MiniCPM-o 优先自主输出 `listen/speak`。能量检测只产生**候选**，绝不直接废弃轮次：
  ...
  候选确认后先**静音验证（duck-and-verify）**：清空设备队列但保留未播尾音，约 350 ms
  内扬声器已静、回声路径消失，post-AEC 仍有持续人声才提交破坏性清空并发送
  `force_listen`；否则原位续播尾音（误触发的代价从整轮 12 秒变成一次短促顿挫），
  同时抬高学习到的残余水平避免同电平反复触发。DoA speech 是 PRE-AEC 信号，机器人
