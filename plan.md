...
- MiniCPM-o 优先自主输出 `listen/speak`。播放开始后的 600 ms 用于学习 far-end 残余；此后
  只有 DoA speech 与 post-AEC 电平连续 5 次超过残余底噪至少 6 dB，才清空播放并发送
  `force_listen`。安静的普通 `listen` 保留短缓冲句尾，已确认的用户插话则清空 GStreamer。
