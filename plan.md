...
- YRobot 使用 DoA 做朝向和连续的轻量姿态叠加，但 DoA 不参与 listen/speak 或打断判断；
  按 SDK 的实时控制原则使用唯一的 `set_target()` 控制点，当前为 50 Hz，相位对齐并按实际
  `dt` 限速。
...
- MiniCPM-o 自主输出 `listen/speak`；DoA 只做朝向，不再触发本地 `force_listen` 或清空播放
  队列。模型进入 `listen` 后不再生成新语音，已经生成的短缓冲正常播完，避免句尾被截断。
