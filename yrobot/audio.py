...
class AudioEngine:
    ...
    def next_audio_unit(self, timeout: float) -> tuple[np.ndarray, bool] | None:
        """Return exactly one second of channel-zero mic audio and force latch."""
    ...
    def latest_frame_jpeg(self) -> bytes | None:
        """Return the latest immutable JPEG snapshot without touching the camera."""
        ...
        while not self._stop_event.is_set():
            try:
                ...
                while capture_pending.size >= VAD_FRAME_SAMPLES:
                    ...
                    if interrupted:
                        control_unit = np.zeros(UPLINK_UNIT_SAMPLES, dtype=np.float32)
                        control_unit[: uplink_pending.size] = uplink_pending
                        uplink_pending = np.empty(0, dtype=np.float32)
                        self._put_uplink(control_unit)
                        with self._lock:
                            self._metrics["force_control_units"] += 1
                    elif uplink_pending.size >= UPLINK_UNIT_SAMPLES:
                        unit = uplink_pending[:UPLINK_UNIT_SAMPLES].copy()
                        uplink_pending = uplink_pending[UPLINK_UNIT_SAMPLES:]
    ...
    def _camera_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            next_capture = max(next_capture + self._camera_period, finished)
