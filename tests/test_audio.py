...
def test_runtime_can_emit_500_ms_uplink_units() -> None:
    media = FakeMedia()
    engine = make_engine(media, uplink_unit_samples=8_000, uplink_queue_size=3)
    source = np.linspace(-0.2, 0.2, 24_000, dtype=np.float32)
    engine.start()
    try:
        media.feed_mono(source)
        wait_until(lambda: engine.metrics["uplink_units"] == 3)

        units = [engine.next_audio_unit(0.1) for _ in range(3)]
        assert all(unit is not None for unit in units)
        joined = np.concatenate([unit[0] for unit in units if unit is not None])
        np.testing.assert_array_equal(joined, source)
        assert all(unit is not None and unit[0].shape == (8_000,) for unit in units)
    finally:
        engine.stop()
...
def test_barge_in_flushes_and_rejects_late_audio_from_old_response() -> None:
    ...
    try:
        ...
        # waiting for the normal configured capture unit to fill.
        ...
        # Subsequent near-end speech remains continuous in ordinary configured
...
def test_camera_snapshot_is_attached_at_most_once() -> None:
    media = FakeMedia()
    engine = AudioEngine(
        FakeMini(media),
        vad=EnergyVad(),
    ...
    engine.start()
    try:
        wait_until(lambda: engine.metrics["camera_frames_captured"] >= 1)
        assert engine.latest_frame_jpeg() == b"jpeg"
        assert engine.latest_frame_jpeg() is None
        assert engine.metrics["camera_frames_consumed"] == 1
    finally:
        engine.stop()
...
def test_response_deltas_and_ordinary_listen_preserve_tail() -> None:
    media = FakeMedia()
    engine = make_engine(media, playback_lead_seconds=0.060)
    engine.start()
    try:
...
def test_interrupted_turn_accepts_new_reply_after_user_release() -> None:
    media = FakeMedia()
    engine = make_engine(media, playback_lead_seconds=0.060)
    engine.start()
    try:
        wait_until(lambda: media.audio.clear_calls >= 1)
        engine.handle_audio_delta(np.full(7_200, 0.12, dtype=np.float32), "old-response")
        wait_until(lambda: engine.state == "speaking")

        media.feed_mono(np.full(2 * 320, 0.18, dtype=np.float32))
        wait_until(lambda: engine.state == "interrupted")
        engine.handle_listen("forced-listen")

        stale_before = engine.metrics["stale_playback_frames"]
        engine.handle_audio_delta(np.full(2_400, 0.7, dtype=np.float32), "old-response")
        assert engine.metrics["stale_playback_frames"] > stale_before

        media.feed_mono(np.zeros(10 * 320, dtype=np.float32))
        wait_until(engine.ready_for_rollover)
        pushed_before = len(media.push_history)

        engine.handle_audio_delta(np.full(4_800, 0.35, dtype=np.float32), "new-response")
        wait_until(lambda: len(media.push_history) > pushed_before)
        assert engine.state == "speaking"
    finally:
        engine.stop()
...
def test_server_wall_clock_metrics_are_retained_for_field_diagnosis() -> None:
    ...
    try:
        engine.handle_audio_delta(
            ...
            "slice",
            {"wall_clock_ms": 1_250.0, "kv_cache_length": 6_321},
        )
        ...
        assert snapshot["last_server_kv_cache_length"] == 6_321.0
        assert snapshot["max_server_kv_cache_length"] == 6_321.0
    finally:
        engine.stop()
