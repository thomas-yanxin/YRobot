...
| Barge-in | Client-owned and destructive: a qualified 200 ms double-talk onset advances the playback epoch and asks the SDK to `clear_player()` immediately, so old queued audio is never resumed. Output stays suppressed and every complete input carries `force_listen` until the exact actually-sent `input_id` returns `listen`; 450 ms of user quiet then admits the new answer. `response_id` is deliberately not treated as a turn boundary | `turn.py`, `audio.py`, `main.py` |
| False triggers from its own echo/motors | Startup applies the Reachy conversation app's verified XVF3800 profile. For every VAD candidate, YRobot searches the exact recently scheduled speaker PCM across the variable capture latency. A candidate is rejected only when correlation is high **and** less than −42 dB of near-end energy remains unexplained, so the real −40 dB interjection seen in hardware logs still passes. Rejected continuous echo is rechecked every 100 ms; a 40 ms gap tolerance preserves speech flicker from XVF double-talk suppression, while the adaptive floor still absorbs steady motor noise | `audio.py`, `main.py` |
