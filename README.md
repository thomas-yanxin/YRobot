...
约 120 ms；软件 FIFO 不丢服务器音频，只有真实插话或关机会清空。插话检测约
`80 ms`。首播日志拆分 `raw_to_enqueue`、`enqueue_to_push`、`resample` 和
`media_push`，服务端日志给出 prefill / generate / wall 与相对下行耗时。所有 YRobot
日志异步写出，不阻塞 WebSocket、音频或动作线程。`time_since_last_uplink_ms` 仅表示
